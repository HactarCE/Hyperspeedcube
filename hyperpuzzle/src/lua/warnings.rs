...
struct WarningSet(HashSet<Cow<'static, str>>);
...
pub(crate) fn first_warning(lua: &Lua, warning_id: impl Into<Cow<'static, str>>) -> bool {
    match lua.app_data_mut::<WarningSet>() {
        Some(mut warning_set) => warning_set.0.insert(warning_id.into()),
        None => {

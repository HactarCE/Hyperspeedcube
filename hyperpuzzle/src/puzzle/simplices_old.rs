...
impl<'a> Simplexifier<'a> {
    ...
    fn radius_upper_bound(&self, shape: ShapeId) -> Result<Float> {
        ...
        while let Some(elem) = queue.pop() {
            let shape_manifold = &self.space[elem].manifold;

            let r = if self.space[elem].ndim()? == 0 {
                // Point pair
                ...
                for child in self.space[shape].boundary.iter() {
    ...
    pub fn face_polygons(&mut self, shape: ShapeId) -> Result<Vec<[VertexId; 3]>> {
        ...
        ensure!(self.space[shape].ndim()? == 2);
    ...
    fn shape_simplices(&mut self, shape: ShapeId) -> Result<SimplexBlob> {
        ...
        let shape_ndim = self.space[shape].ndim()?;
        let shape_manifold = &self.space[shape].manifold;

        let is_shape_flat = shape_manifold.is_flat();
        let are_all_children_flat = self.space[shape].boundary.iter().all(|boundary_elem| {
            let boundary_elem = &self.space[boundary_elem.id];
        ...
        for boundary_elem in self.space[shape].boundary.iter() {
            let cut = self.space.signed_manifold_of_shape(boundary_elem)?;
            ...
            let mut slice_op = SimplexSliceOp {
                ...
                vertex_split_cache: HashMap::new(),
                simplex_split_cache: HashMap::new(),
    ...
    fn flat_shape_simplices(&mut self, shape: ShapeId) -> Result<SimplexBlob> {
        ...
        if shape_ndim == 1 {
            let boundary_elems = &self.space[shape].boundary;
            ensure!(
            ...
            let [a, b] = self.space[boundary_elem.id].manifold.to_point_pair()?;
            Ok(Simplex::new([
    ...
    fn new_sphere(
        ...
        let m = Manifold::new_hypersphere(center, radius, self.space.space().ndim()?);
    ...
    fn subdivide_simplex(
        ...
        s: &Simplex,
        m: &Manifold,
        cache: &mut HashMap<Simplex, SimplexBlob>,
    ) -> Result<SimplexBlob> {
    ...
    fn cut_edge_uncached(
        ...
        if inside.is_empty() && outside.is_empty() {
            ...
            if !outside.is_empty() {
                ...
                let intersection_point = match line.split(&op.cut, self.space.space())? {
                    ManifoldSplit::Split {
    ...
    fn split_vertex(&mut self, op: &mut SimplexSliceOp<'_>, v: VertexId) -> Result<PointWhichSide> {
        ...
        let result = op.cut.which_side_has_point(&point, self.space.space())?;
...
struct SimplexSliceOp<'a> {
    ...
    vertex_split_cache: HashMap<VertexId, PointWhichSide>,
    /// Cached results of splitting simplices.
    simplex_split_cache: HashMap<Simplex, SimplexSplit>,
    /// Cached results of subdividing simplices.
    subdivision_cache: HashMap<Simplex, SimplexBlob>,
}
...
mod tests {
    ...
    fn test_cube_simplexification() {
        ...
        let mut space = Space::new_euclidean_cga(NDIM);
        for ax in 0..NDIM {
            let v = Vector::unit(ax);
            for sign in [Sign::Neg, Sign::Pos] {
                space.carve_plane(&v * sign.to_float(), 1.0, 0).unwrap();
            }
            space.slice_plane(v, 0.0).unwrap();
        }
        println!("{space}");

        let mut simplexifier = Simplexifier::new(&space);
        for &shape in space.roots() {

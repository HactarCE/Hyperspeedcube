...
impl Manifold {
    ...
    pub fn from_opns(opns: Blade, space_ndim: u8) -> Result<Self> {
        Ok(Self {
            space_ndim,
    ...
    /// degenerate.
    pub fn from_ipns(ipns: &Blade, space_ndim: u8) -> Result<Self> {
    ...
    pub fn new_point_pair(a: &Point, b: &Point, space: &Self) -> Result<Self> {
        Manifold::from_opns(Blade::point(a) ^ Blade::point(b), space.space_ndim)
            .context("constructing point pair")
    }
    /// Constructs a line (represented by a 1D manifold).
    pub fn new_line(a: &Point, b: &Point, space: &Self) -> Result<Self> {
        Manifold::from_opns(
    ...
    pub fn new_hyperplane(normal: impl VectorRef, distance: Float, space_ndim: u8) -> Self {
        Manifold::from_ipns(&Blade::ipns_plane(normal, distance), space_ndim).unwrap()
    }
    /// Constructs a spherical manifold.
    pub fn new_hypersphere(center: impl VectorRef, radius: Float, space_ndim: u8) -> Self {
        Manifold::from_ipns(&Blade::ipns_sphere(center, radius), space_ndim).unwrap()
    }
    ...
    pub fn to_point_pair(&self) -> Result<[Point; 2]> {
    ...
    /// `self`.
    pub fn which_side_has_point(&self, p: &Point, space: &Self) -> Result<PointWhichSide> {
        Ok(self.ipns_in_space(space).ipns_query_point(p))
    }

    /// Returns a function which computes the span of a tangent space of a given
    /// on the manifold.
    pub fn tangent_space(&self) -> Result<Box<dyn TangentSpace<Point>>> {
        if self.is_flat() {
    ...
    /// undefined.
    pub fn project_point(&self, p: &Point) -> Result<Option<Point>> {
    ...
    fn test_cga_euclidean_intersection_orientation() {
        ...
        for ndim in 1..=8 {
            ...
            for subspace_ndim in 1..ndim {
                let subspace =
                    Manifold::from_opns(Blade::pseudoscalar(subspace_ndim), ndim).unwrap();
                ...
                for subsubspace_ndim in 1..subspace_ndim {
                    let subsubspace =
                        Manifold::from_opns(Blade::pseudoscalar(subsubspace_ndim), ndim).unwrap();

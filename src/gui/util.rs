...
pub(super) struct PlaintextYamlEditor {
    pub id: egui::Id,

    // TODO: consolidate this square button size into a global if it's the same
    // everywhere
...
impl PlaintextYamlEditor {
        ...
        T: Serialize + for<'de> Deserialize<'de> + Clone,
    {
        self.state(ui).map(|mut state| {
            ...
            let square_button = |ui: &mut egui::Ui, text: &str, hover_text: &str| {
            ...
            let mut r = ui.scope(|ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.add_enabled_ui(parsed_value.is_ok(), |ui| {
                        if square_button(ui, "✔", "Confirm changes").clicked() {
                            self.set_state(ui, None);
                            *value = parsed_value.as_ref().unwrap().clone();
                            changed = true;
                        }
                    });
                    if square_button(ui, "✖", "Discard changes").clicked() {
                        self.set_state(ui, None);
                    }

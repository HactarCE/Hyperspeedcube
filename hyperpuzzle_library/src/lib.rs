...
pub fn verify(log_file: &LogFile) -> Vec<SolveVerification> {
    log_file
        .solves
        .iter()
        .filter_map(|solve| {
            ...
            let mut speedsolve_duration = None;
            ...
            for event in log {
                match event {
                    ...
                    LogEvent::EndSolve { time, duration } => {
                        time_completed = Some(*time);
                        speedsolve_duration = *duration;
            ...
            Some(SolveVerification {
                program: log_file.program.clone(),
                ...
                speedsolve_duration: speedsolve_duration.map(Duration::milliseconds),
...
pub struct SolveVerification {
    pub program: Option<hyperpuzzle_log::Program>,

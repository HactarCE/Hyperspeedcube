...
//     };
//     for node in doc.nodes() {
//         parse_lang_node(&src, schema, &mut lang, node, "", LANG_STRUCT_NAME);
//     }
//     lang
// }
...
//         for (field_name, field_data) in struct_schema.fields.clone() {
//             if field_data.may_be_inline {
//                 let field_path = format!("{new_prefix}{}", field_data.name);
//                 parse_lang_inline_field(src, schema, lang, &mut entries, field_path);
//             }
//         }
//         util::ignore_entries(src, entries);
...
//         field_ty = StructFieldType(Some(new_struct_name.to_owned()));
...
//         field_ty = StructFieldType(Some(new_struct_name));
//     } else {
//         let mut entries = node.entries().iter();
...
//             return;
//         };
//         let v = util::take_entry_str_value(&src, entry).and_then(|s| parse_lang_value(schema, s));
//         insert_lang_value(lang, path.clone(), v.clone(), || {
//             src.at(entry.span().offset())
//         });
//         util::ignore_entries(src, entries);

//         field_ty = StructFieldType(match v {
//             Some(LangValue::Template { .. }) => Some(schema.key_to_struct_name(&path)),
//             _ => None,
//         });
//     }
...
//             lang,
//             path,
//             util::take_entry_str_value(src, entry).and_then(|s| parse_lang_value(schema, s)),
//             || src.at(entry.span().offset()),
//         );
//     }
// }

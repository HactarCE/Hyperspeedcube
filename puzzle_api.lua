...
add_shape {
  ...
  facet_order = {'Right', 'Left', 'Up', 'Down', 'Front', 'Back'},
  facet_colors = {
    Right = colors.red,
    Left = colors.orange,
    Up = colors.white,
...
cubic = twists {
  ...
  generate = function(p)
    ...
    -- Define rotations
    p:twist_direction('x', rot {
      bivector = ~X,
      order = 4
    })
    p:twist_direction('y', rot {
      bivector = ~Y,
      order = 4
    })
    p:twist_direction('z', rot {
      bivector = ~Z,
      order = 4
    })

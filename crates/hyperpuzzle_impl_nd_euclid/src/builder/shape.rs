...
impl ShapeBuilder {
    ...
    pub fn new_empty(
        ...
        Ok(Self {
            space: Space::with_primordial_cube_radius(ndim, primordial_cube_radius)?,
    ...
    pub fn build(&self, warn_fn: &mut impl FnMut(eyre::Error)) -> Result<ShapeBuildOutput> {
        ...
        for old_piece_id in self.active_pieces.iter() {
            ...
            let sticker_shrink_vectors = space.get(piece.polytope).sticker_shrink_vectors(
                &stickers_of_piece
                    .iter()

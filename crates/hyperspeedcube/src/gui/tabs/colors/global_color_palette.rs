...
fn show_custom_colors_section(mut prefs_ui: PrefsUi<'_, GlobalColorPalette>) {
    ...
    ui.horizontal(|ui| {
        if ui.button(L.colors.actions.add).clicked() {
            ...
            // Ignore saved name because there's no way to "activate" a custom color.
            let _saved_color_name = custom_colors.save_preset(name, Rgb { rgb });
            ...
            *prefs.changed |= r.changed();
            ...
            let popup_response = popup.if_open(|popup| {
                popup
                    ...
                    .over(ui, &label_response, egui::vec2(1.0, 4.0)) // overwrite width if wider

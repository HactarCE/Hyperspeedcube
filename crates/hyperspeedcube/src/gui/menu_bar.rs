    ...
    egui::MenuBar::new().ui(ui, |ui| {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                ...
                ui.separator();
                let lb_state = &app_ui.app.leaderboards;
                let mut lb_state_guard = lb_state.lock();
                let mut wants_sign_out = false;
                let leaderboards_msg = match &*lb_state_guard {
                    LeaderboardsClientState::NotSignedIn => {
                        ui.menu_button("Leaderboards sign-in", |ui| {
                            ...
                            if ui.link("Sign into the leaderboards").clicked() {
                                let url = lb_state_guard.init_auth(Arc::clone(&lb_state));

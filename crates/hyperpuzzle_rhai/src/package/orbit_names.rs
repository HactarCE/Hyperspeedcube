...
/// symmetry group.
pub fn names_from_table<I: IndexNewtype>(
    ctx: &Ctx<'_>,
    map: Map,
) -> Result<Vec<(NameSpec, Vec<u8>)>> {
    ...
    for (k, v) in map {
        ...
        let gen_seq: Vec<u8> = seq.into_iter().map(|v| from_rhai(ctx, v)).try_collect()?;
        let id = gen_seqs
    ...
    let key_value_dependencies: GenericVec<I, (Vec<u8>, Option<I>)> =
    ...
    .into_iter()
    .filter_map(|(k, v)| Some((names.get(k)?.clone(), v)))
    .collect())
}

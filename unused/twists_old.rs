...
use hyperpuzzle_core::{
    Axis, AxisSystem, CatalogId, CatalogObject, ComponentList, IndexOverflow, NameSpecBiMap,
    NameSpecBiMapBuilder, Names, Orbit, PerAxis, TwistSystem, TypedIndex, TypedIndexIter,
};
...
use crate::names::{FactorTwistSystemNames, PrefixFreeTrie, ProductTwistSystemNames};
use crate::{
...
struct AxisInOrbit(Axis);
struct AxisInFactor(Axis);

struct NamedPointInOrbit(NamedPoint);
struct NamedPointInFactor(NamedPoint);

#[derive(Debug, Clone)]
pub struct TwistSystemFactorNames {
    named_points: Vec<Str>,
    // axes: Vec<PermutationPatternString>,
}
...
pub(crate) struct TwistSystemProduct {
    ...
    /// Names for named points and axes, for each factor.
    pub factor_names: Vec<TwistSystemFactorNames>,
...
impl TwistSystemProduct {
    ...
    pub fn new_factor(
        ...
        let mut named_point_from_name = HashMap::new();
        let mut new_named_point_orbits = vec![];
        let mut id_offset = 0;
        for orbit in named_point_orbits {
            let mut names = vec![];
            for (vector, name, _gen_seq) in &orbit.named_point_vectors {
                let id = named_point_vectors.push(vector.clone())?;
                names.push(name.clone());
                named_point_from_name.insert(name.clone(), id);
            }
        ...
        let get_named_point_from_name = |name: &Str| {
            named_point_from_name
                .get(&**name)
                .copied()
        ...
        let named_point_set_orbits: Vec<(NamedPointSet, Float)> = std::iter::chain(
            ...
            named_point_set_orbits.iter().map(|(names, distance)| {
                eyre::Ok((
                    NamedPointSet::new(names.iter().map(get_named_point_from_name).try_collect()?)?,
                    *distance,
                ))
            }),
        )
    ...
    pub fn direct_product(&self, rhs: &Self) -> Result<Self> {
        ...
        let b_new_named_point_set_orbits: Vec<_> = b
            .named_point_set_orbits
            .iter()
            .map(|(set, distance)| Ok((lift_b_point_set(set)?, *distance)))
            .try_collect()?;
    ...
    fn build_named_point_names(&self) -> Result<Names<NamedPoint>> {
        let needs_prefix = self
            .named_point_orbits
            .iter()
            .map(|o| o.prefix.0)
        ...
        let mut names = PerNamedPoint::<Str>::new();
        for orbit in &self.named_point_orbits {
            for name in &**orbit.names {
                let prefixed_name = if needs_prefix {
                    format!("{}{}", orbit.prefix, name).into()
                } else {
                    name.clone()
...
pub struct AxisOrbit {
    ...
    pub names: Arc<Vec<PermutationPatternString>>,
